...
每个 Chunk 应控制在 **reviewer 友好** 的范围内：

- **理想**: 100–300 行（含测试），3–8 个文件
- **上限**: 400 行或 12 个文件——超过时考虑进一步拆分
- 目标：reviewer 在 15–30 分钟内能完成 review
...
| #   | Chunk  | 模式  | 依赖 | 预估（含测试） | 状态 | PR  | Branch                 | Review 要点 |
| --- | ------ | ----- | ---- | -------------- | ---- | --- | ---------------------- | ----------- |
| 1   | {name} | 🎨/⚙️ | 无   | ~N 行          | ⏳   | —   | feat/{task-id}-chunk-1 | {focus}     |
| 2   | {name} | 🎨/⚙️ | #1   | ~N 行          | ⏳   | —   | feat/{task-id}-chunk-2 | {focus}     |
| ... |        |       |      |                |      |     |                        |             |
