...
#### Stage 6 → 委派给 @git-worker 提交并创建 PR（三阶段确认）

验证通过后，Stage 6 分为三个阶段，确保用户在主对话线程中看到 commit message 和 PR 内容：
...
Dev 从 git-worker 返回的 `DRAFT_RESULT` 块中提取内容，**在主对话回复正文中**展示格式化的 commit message 和 PR 内容。示例：
